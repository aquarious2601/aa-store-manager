...
| Permission denied            | # Traverse into /home/ubuntu for non-ubuntu users <br/> `sudo chmod o+x /home/ubuntu` <br/> `sudo chmod -R o+rX /home/ubuntu/aashop/frontend/dist` <br/> `sudo chmod -R o+rX /home/ubuntu/aashop/backend/public` <br/> `sudo systemctl reload nginx`                                                                                                                                                                                          |
| Playwright permission denied | `sudo mkdir -p /opt/ms-playwright` <br/> `sudo chown -R ubuntu:www-data /opt/ms-playwright` <br/>`sudo chmod -R g+rwXs /opt/ms-playwright`<br/># Install once into the shared location<br/>`PLAYWRIGHT_BROWSERS_PATH=/opt/ms-playwright /home/ubuntu/aashop/scraper/.venv/bin/playwright install chromium`<br/># Persist for all future shells<br/> `echo 'PLAYWRIGHT_BROWSERS_PATH=/opt/ms-playwright' \| sudo tee -a /etc/environment` |
